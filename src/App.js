...
const App = () => {
  ...
  async function askContractToMintNftWhitelist(quantity, _merkleProof) {
    try {
        setloading(true)
      ...
      try{
        ...
        if(error == "Error: Returned error: execution reverted: Whitelist mint is not active yet"){
          Swal.fire({
            icon: 'error',
            title: 'Minting Failed',
            text: 'Whitelist mint has not started yet',
           
          })          
        }else if(error == "Error: Returned error: execution reverted: ERC721A: quantity to mint too high"){
          Swal.fire({
            icon: 'error',
            title: 'Minting Failed',
          ...
          Swal.fire({
            icon: 'error',
            title: 'Minting Failed',
            text: 'Invalid MerkleProof - You are not on the whitelist',
